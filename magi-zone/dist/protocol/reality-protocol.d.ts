 ...
 */
export declare class RealityProtocol {
    /**
     * Parse raw input (voice/will + wave state) into structured invocations
     * and pattern ids for the enforce protocol.
     */
    parseViaHypercomputation(voicePhrases: string[], waveState: DimensionalWaveState, resolveIntent: (phrase: string, energySignature: number[]) => InvocationIntent): HypercomputationParseResult;
    /**
     * Build scanned area (radar-like): individual scanned area to create
     * dimensional invocation – 2D in 3D spacing – for reality tear.
     */
    buildScannedArea(center: [number, number, number], radius: number, intents: InvocationIntent[]): ScannedArea;
}
